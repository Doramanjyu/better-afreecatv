...
const checkNotifications = async () => {
  try {
    ...
    const respCount = await fetch('https://afevent.afreecatv.com/api/notification.php?szWork=getBadgeInfo',
      {
        method: "GET",
      },
    )
    ...
    const resp = await fetch('https://afevent.afreecatv.com/api/notification.php?szWork=getNotifications&szWork=getNotifications&nPageNo=1',
      {
        method: "GET",
      },
    )
    ...
    let latestSeq = (await ubrowser.storage.local.get('latestSeq'))?.latestSeq || 0
    console.log(`last notified seq: ${latestSeq}`)

    const ops = data.DATA.reverse().map((d) => {
      if (d.READ_FLAG === '1' || d.FROM_ID === 'afnotice' || d.NOTI_TYPE !== 'F01' || d.SEQ <= latestSeq) {
        return null
      }
      latestSeq = d.SEQ
      return () => {
